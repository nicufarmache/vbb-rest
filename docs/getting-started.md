# Getting Started with `v6-vbb.nicu.ro`
...
```shell
curl 'https://v6-vbb.nicu.ro/locations?poi=false&addresses=false&query=südkreuz' -s | jq
```
...
```shell
curl 'https://v6-vbb.nicu.ro/stops/900058101/departures?results=5' -s | jq
```
...
```shell
curl 'https://v6-vbb.nicu.ro/journeys?from=900058101&to=900110005&departure=tomorrow+2pm&results=2' -s | jq
```
...
These are the basics. Check the full [API docs](api.md) for all features or use the [OpenAPI playground](https://petstore.swagger.io/?url=https%3A%2F%2Fv6-vbb.nicu.ro%2F.well-known%2Fservice-desc%0A) to explore the API!
