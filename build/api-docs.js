...
const HEAD = `\
# \`v6-vbb.nicu.ro\` API documentation

[\`v6-vbb.nicu.ro\`](https://v6-vbb.nicu.ro/) is a [REST API](https://restfulapi.net). Data is being returned as [JSON](https://www.json.org/).
...
\`\`\`shell
curl 'https://v6-vbb.nicu.ro/locations?query=alexanderplatz&results=1' -s | jq
\`\`\`
...
\`\`\`shell
curl 'https://v6-vbb.nicu.ro/locations/nearby?latitude=52.52725&longitude=13.4123' -s | jq
\`\`\`
...
\`\`\`shell
curl 'https://v6-vbb.nicu.ro/stops/reachable-from?latitude=52.52446&longitude=13.40812&address=10178+Berlin-Mitte,+Münzstr.+12' -s | jq
\`\`\`
...
\`\`\`shell
curl 'https://v6-vbb.nicu.ro/stops/900017101' -s | jq
\`\`\`
...
curl 'https://v6-vbb.nicu.ro/stops/900013102/departures?direction=900014101&duration=10' -s | jq
\`\`\`
...
curl 'https://v6-vbb.nicu.ro/stops/900013102/arrivals?duration=10' -s | jq
\`\`\`
`,
	'/stations': `\
### Examples
...
curl 'https://v6-vbb.nicu.ro/stations?query=mehringd' -s | jq
\`\`\`
...
\`\`\`shell
# filter vbb-stations
curl 'https://v6-vbb.nicu.ro/stations?location.latitude=52.493567' -s | jq
\`\`\`
...
curl 'https://v6-vbb.nicu.ro/stations?location.latitude=52.493567' -H 'accept: application/x-ndjson' -s | jq
\`\`\`
`,
	'/stations/:id': `\
### Example
...
curl 'https://v6-vbb.nicu.ro/stations/de:11000:900017101' -s | jq
\`\`\`
...
\`\`\`shell
# stop/station to POI
curl 'https://v6-vbb.nicu.ro/journeys?from=900023201&to.id=900980720&to.name=ATZE+Musiktheater&to.latitude=52.54333&to.longitude=13.35167' -s | jq
# without buses, with ticket info
curl 'https://v6-vbb.nicu.ro/journeys?from=…&to=…&bus=false&tickets=true' -s | jq
\`\`\`
`,
	'/journeys/:ref': `\
### Example
...
journey=$(curl 'https://v6-vbb.nicu.ro/journeys?from=…&to=…&results=1' -s | jq '.journeys[0]')
refresh_token=$(echo $journey | jq -r '.refreshToken')

# refresh the journey
curl "https://v6-vbb.nicu.ro/journeys/$(echo $refresh_token | url-encode)" -s | jq
\`\`\`
`,
	'/trips/:id': `\
### Example
...
# fetch the trip
curl "https://v6-vbb.nicu.ro/trips/$(echo $trip_id | url-encode)" -s | jq
\`\`\`
`,
	'/radar': `\
### Example
...
curl "https://v6-vbb.nicu.ro/radar?$bbox&results=10" -s | jq
\`\`\`
`,
	'/lines': `\
### Example
...
\`\`\`shell
curl 'https://v6-vbb.nicu.ro/lines?operator=796&variants=false' -s | jq
\`\`\`
`,
	'/lines/:id': `\
### Example
...
\`\`\`shell
curl 'https://v6-vbb.nicu.ro/lines/17442_900' -s | jq
\`\`\`
`,
	'/maps/:type': `\
### Example
